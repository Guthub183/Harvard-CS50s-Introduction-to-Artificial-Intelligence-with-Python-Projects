...
class MinesweeperAI():
            ...
            for c in range(j - 1, j + 2):
                ...
                if 0 <= r < self.height and 0 <= c < self.width:
                    if (r, c) in self.mines:
                        mine_count += 1
                    elif (r, c) in self.safes:
                        continue
                    else:
                        neighbors.add((r, c))

        # Add the new sentence (adjusting count for already known mines)
        new_sentence = Sentence(neighbors, count - mine_count)
        self.knowledge.append(new_sentence)

        # 4 & 5. Loop to update knowledge and make inferences iteratively
        changed = True
        while changed:
            changed = False
            ...
            if new_safes or new_mines:
                ...
                changed = True
                # Clean up empty sentences immediately
            ...
            for s1 in self.knowledge:
                for s2 in self.knowledge:
                    if s1 == s2:
                        continue
                    # If s1 is a subset of s2, we can infer s2 - s1
            ...
            # Clean up empty sentences and remove duplicates
            self.knowledge = [s for s in self.knowledge if len(s.cells) > 0]
            unique_knowledge = []
    ...
    def make_random_move(self):
        ...
        if len(movable_cells) > 0:
            return random.choice(movable_cells)
        return None
